...
const messages = defineMessages({
  password_reset_email_sent: {
    ...
    description:
      "Notification that the email with password reset instructions has been sent",
  },
  password_reset_sms_sent: {
    id: "password_reset_sms_sent",
    defaultMessage: "A text message with security code has been sent.",
    description:
      "Notification that the SMS with password reset instructions has been sent",
  },
});
...
class PasswordResetView extends React.PureComponent {
  ...
  componentDidUpdate(prevProps, prevState) {
    if (
      prevProps.reqCredMethod != this.props.reqCredMethod &&
      this.props.reqCredMethod
    ) {
      ...
      this.props.onRequest(this.props.reqCredMethod, email || tel).then((_) => {
        ...
        const msg =
          this.props.reqCredMethod == "email"
            ...
            ? messages.password_reset_sms_sent
            : null;
        if (msg) {
  ...
  render() {
    const showCredentialInput = !(
      (this.state.token || (this.state.code && this.props.reqCredMethod)) &&
      this.state.scheme
    );
    const showPasswordInput =
      !showCredentialInput || this.state.haveCode || this.state.sent;
